    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Manage student records for your institution</p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <input type="text" placeholder="Filter by grade" value={gradeFilter} onChange={(e) => { setGradeFilter(e.target.value); setPage(0); }}
          className="w-40 px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 bg-white text-slate-800" />
      </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                <motion.tr key={s.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 text-slate-600">{s.grade}{s.section ? `-${s.section}` : ''}</td>
                  <td className="px-4 py-3 capitalize text-slate-600">{s.gender?.toLowerCase()}</td>
                  <td className="px-4 py-3 text-slate-600">{s.dateOfBirth}</td>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
              ...
              <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
                ...
                <button onClick={() => setShowForm(false)} className="p-1 rounded-md hover:bg-slate-100"><X className="w-5 h-5 text-slate-500" /></button>
              </div>
