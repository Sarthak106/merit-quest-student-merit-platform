    ...
    <div className="space-y-6">
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="flex items-center justify-center w-12 h-12 rounded-xl bg-indigo-50 text-indigo-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <motion.div custom={4} initial="hidden" animate="visible" variants={cardVariants} className="card">
          <h3 className="text-lg font-semibold text-slate-900 mb-4">Score by Grade</h3>
          {gradeDistribution.length > 0 ? (
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={gradeDistribution}>
                <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
                <XAxis tick={{ fill: "#64748b" }} axisLine={{ stroke: "#e2e8f0" }} dataKey="grade" />
            ...
            <p className="text-slate-400 text-center py-12">No grade distribution data available</p>
          )}
        </motion.div>

        {/* Score Histogram */}
        <motion.div custom={5} initial="hidden" animate="visible" variants={cardVariants} className="card">
          <h3 className="text-lg font-semibold text-slate-900 mb-4">Composite Score Distribution</h3>
          {scoreHistogram.length > 0 ? (
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={scoreHistogram}>
                <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
                <XAxis tick={{ fill: "#64748b" }} axisLine={{ stroke: "#e2e8f0" }} dataKey="range" fontSize={12} />
                <YAxis tick={{ fill: "#64748b" }} axisLine={{ stroke: "#e2e8f0" }} />
                <Tooltip contentStyle={{ backgroundColor: "#ffffff", border: "1px solid #e2e8f0", borderRadius: "12px", color: "#1e293b" }} />
            ...
            <p className="text-slate-400 text-center py-12">No score data available</p>
          )}
        </motion.div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <motion.div custom={6} initial="hidden" animate="visible" variants={cardVariants} className="card">
          <h3 className="text-lg font-semibold text-slate-900 mb-4">Subject Performance ({academicYear})</h3>
          {subjectPerformance.length > 0 ? (
            ...
            <p className="text-slate-400 text-center py-12">No subject data available</p>
          )}
        </motion.div>
      ...
      {isGovOrAdmin && institutionComparison.length > 0 && (
        <motion.div custom={8} initial="hidden" animate="visible" variants={cardVariants} className="card">
          <h3 className="text-lg font-semibold text-slate-900 mb-4">Institution Comparison</h3>
          <ResponsiveContainer width="100%" height={350}>
            <BarChart data={institutionComparison} layout="vertical">
              <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
              <XAxis type="number" />
              <YAxis tick={{ fill: "#64748b" }} axisLine={{ stroke: "#e2e8f0" }} dataKey="institutionName" type="category" width={150} fontSize={12} />
              <Tooltip contentStyle={{ backgroundColor: "#ffffff", border: "1px solid #e2e8f0", borderRadius: "12px", color: "#1e293b" }} />
              <Legend wrapperStyle={{ color: "#64748b" }} />
                ...
                {topPerformers.map((tp, idx) => (
                  ...
                  >
                    ...
                    <td className="table-cell">
                      <div className="font-medium text-slate-800">{tp.studentName}</div>
                      <div className="text-xs text-slate-500">{tp.enrollmentNumber}</div>
                    </td>
                    <td className="table-cell">{tp.grade}</td>
                    {isGovOrAdmin && <td className="table-cell text-slate-500">{tp.institutionName}</td>}
                    <td className="table-cell font-semibold text-indigo-700">{tp.compositeScore?.toFixed(4)}</td>
