    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-white/50 mt-1">Manage registered educational institutions</p>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-white/5">
              {loading ? (
                ...
                <motion.tr key={inst.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="hover:bg-white/5">
                  <td className="px-4 py-3 font-medium">{inst.name}</td>
                  <td className="px-4 py-3 font-mono text-xs">{inst.code}</td>
                  <td className="px-4 py-3">
                    <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-blue-500/15 text-blue-400">{inst.type}</span>
                  </td>
                  <td className="px-4 py-3 text-white/50">{inst.board || '—'}</td>
                  <td className="px-4 py-3 text-white/50">{inst.district}</td>
                  <td className="px-4 py-3 text-white/50">{inst.state}</td>
                  <td className="px-4 py-3">
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${inst.active !== false ? 'bg-emerald-500/15 text-emerald-400' : 'bg-red-500/15 text-red-400'}`}>
                      {inst.active !== false ? 'Active' : 'Inactive'}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex gap-2">
                      <button onClick={() => openEdit(inst)} className="p-1.5 text-white/40 hover:text-indigo-400 rounded-md hover:bg-indigo-500/10"><Edit2 className="w-4 h-4" /></button>
                      <button onClick={() => handleDeactivate(inst.id)} className="p-1.5 text-white/40 hover:text-red-400 rounded-md hover:bg-red-500/10"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  </td>
                </motion.tr>
              ))}
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-white/60 mb-1">Contact Phone</label>
